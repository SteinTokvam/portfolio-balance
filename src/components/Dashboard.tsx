...
import { Account, EquityType, Holding, KronDevelopment, Transaction, TransactionType } from "../types/Types"
...
export default function Dashboard() {
    ...
    if (Array.from(data.keys()).length < 4) {
        return (
        ...
        const accounts = data.get("accounts") as Account[]
        const holdings = data.get("holdings") as Holding[]
        const transactionsByAccount = data.get("transactions").transactionsByAccount
        const kronDevelopment = data.get("kronDevelopment") as KronDevelopment[]

        console.group("Dashboard")
        console.log("accounts", accounts)
        console.log("holdings", holdings)
        console.log("transactionsByAccount", transactionsByAccount)
        console.log("kronDevelopment", kronDevelopment)
        console.groupEnd()
        ...
        const accountData = accounts.map((account: Account) => {
            ...
            if (account.name === "FundingPartner") {
                ...
                yieldForAccount = !loading && kronDevelopment.length > 0 ? kronDevelopment[kronDevelopment.length - 1].yield_in_currency.toFixed(0) : "0"
